...
Source for **[zpad.wangyitu.tech](https://zpad.wangyitu.tech)** — the official site of [ZPad](https://github.com/pekaboo/ZPad), a native macOS focus panel (terminal + browser, one hotkey away).
...
Custom domain `zpad.wangyitu.tech` is wired via the `CNAME` file + a DNS CNAME record pointing at `neo-idea.github.io` (DNS-only, no proxy).
